...
const LoanForm = ({ isOpen, onClose, onSubmit }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (formData.principal && formData.rate && formData.tenure && formData.loanName) {
      ...
      setFormData({ loanName: '', principal: '', rate: '', tenure: '' });
      onClose();
    }
  };
                  ...
                  {loans.map((loan, index) => {
                    ...
                    return (
                      <tr key={loan.id} className={`hover:bg-gray-50 transition-colors ${index % 2 === 0 ? 'bg-white' : 'bg-gray-25'}`}>
                        ...
                        <td className="px-6 py-4 text-right text-gray-900">{loan.rate}%</td>
                        <td className="px-6 py-4 text-right text-gray-900">{loan.tenure} months</td>
                        <td className="px-6 py-4 text-right text-green-600 font-semibold">${emi.toFixed(2)}</td>
                        <td className="px-6 py-4 text-right text-purple-600 font-semibold">${totalPayable.toFixed(2)}</td>
